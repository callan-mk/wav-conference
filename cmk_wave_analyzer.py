#!/usr/bin/env python
"""
Original Code source: https://github.com/endolith/waveform_analysis
...
# ---------------------------------------------------------------------------
# my version of analysis for A-weighted volume comparison
def rmsA(filename):
    # not touching this bit because COMPLICATED
    if wav_loader == 'pysoundfile':
        sf = SoundFile(filename)
        signal = sf.read()
        channels = sf.channels
    ...
    if channels == 1:
        # Monaural
        results = rmsProperties(signal, sample_rate)
    elif channels == 2:
        # Stereo
        x = []
        x.append(rmsProperties(signal[:, 0], sample_rate))
        x.append(rmsProperties(signal[:, 1], sample_rate))
        results = sum(x) / len(x) 
    else:
        # Multi-channel
        x = []
        for ch_no, channel in enumerate(signal.transpose()):
            x.append(rmsProperties(channel, sample_rate))
        results = sum(x) / len(x) 
        
    return results


# version of properties fn for my rmsA fn
def rmsProperties(signal, sample_rate):
    """
    Return rmsA scaled for a given channel data.
    """
    # Measurements that include DC component
    DC_offset = mean(signal)
    # Maximum/minimum sample value
